...
**Soft margin** (`C` parameter): allow some misclassification, trading margin size against violations. Large `C` = fewer violations (lower bias, more overfit); small `C` = wider margin, more tolerance. `C` is the main tuning knob — see [regularization](regularization.md) for the same bias/variance reasoning.
...
This was the classic "I can do nonlinear classification with linear math" trick — conceptually the ancestor of feature learning (see [neural-network](neural-network.md)).
...
SVM was the pre-deep-learning default for many classification tasks. For tabular data it's still a solid baseline (see [tabular-regression](tabular-regression.md)), but boosting ([XGBoost](XGBoost.md)) and neural nets usually win at scale. The *kernel* idea lives on in kernel methods research.

## Related

- [classification](classification.md) — the task; SVM is one approach.
- [logistic-regression](logistic-regression.md) — its probabilistic rival for linear models.
- [regularization](regularization.md) — the `C` knob is a regularization tradeoff.
