...
1. Compute distance from the query to every training point (usually Euclidean — see the L2 norm in [[../mathematical-function/PLAN]]).
...
- **Curse of dimensionality**: distance becomes meaningless in high dims — points are all roughly equidistant, so "nearest" is noise. The relative contrast `d_max/d_min` → 1 as dimensions grow. This is exactly why [[automated-feature-extraction]] and good features matter.
...
- **Similarity-based retrieval** — the modern survival of k-NN: nearest-neighbor search in learned embedding spaces (see [[metric-learning]] and [[../nlp]] RAG notes). The "k" in RAG retrieval is k-NN over embeddings.

## Related

- [[metric-learning]] — learning the distance k-NN uses (the fix for raw-space k-NN).
- [[classification]] — the task; k-NN is the baseline to beat.
- [[bias-variance-tradeoff]] — small `k` = low bias/high variance; large `k` = the reverse.
