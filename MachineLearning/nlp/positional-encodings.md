...
Transformers have **no inherent notion of order** — attention is a set operation, and "I love you" and "you love I" produce identical attention scores without position information. Positional encodings inject "where am I" into the model (see [[../deeplearning/attention-is-all-you-need]]).
...
- **RoPE (Rotary Position Embedding)** — the default for LLaMA, GPT-NeoX, most modern LLMs: rotate the query/key vectors by an angle proportional to position. It makes the *dot product* (see [[../mathematical-function/dot-product-cosine]]) depend only on the **relative distance** `i-j` — attention naturally decays with distance, and it extrapolates well. This is the "rope" you see in every modern config file.
...
- KV-cache and sliding-window attention (see [[../deeplearning/distributed-training]] / [[GPT-decoder-only-models]]) interact with position handling.

## Related

- [[../deeplearning/attention-is-all-you-need]] — the paper that introduced sinusoidal PE.
- [[GPT-decoder-only-models]] — RoPE/ALiBi in production models.
- [[BERT]] — learnable absolute PE.
