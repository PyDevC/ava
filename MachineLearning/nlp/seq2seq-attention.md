...
Early seq2seq (Sutskever et al. 2014) compressed the entire source into **one fixed vector** (the encoder's last hidden state). Long inputs → the context vector can't hold it all → translation quality collapses on long sentences (the [RNN-LSTM-GRU](../deeplearning/RNN-LSTM-GRU.md) long-term memory problem, again).
...
This is the direct ancestor of the Transformer's self-attention ([attention-is-all-you-need](../deeplearning/attention-is-all-you-need.md)). Two generations:

- **Additive attention** (Bahdanau): `score = W·tanh(Wₐq + W_b k)` — a small MLP.
- **Dot-product attention** (Vaswani): `score = q·k / √d` — the modern standard (see [dot-product-cosine](../mathematical-function/dot-product-cosine.md)).
...
4. T5 / BART — modern encoder-decoder Transformers (see [T5-encoder-decoder](T5-encoder-decoder.md)).

## Related

- [attention-is-all-you-need](../deeplearning/attention-is-all-you-need.md) — the Transformer paper, direct descendant.
- [RNN-LSTM-GRU](../deeplearning/RNN-LSTM-GRU.md) — the encoder/decoder base it replaced.
- [T5-encoder-decoder](T5-encoder-decoder.md) — the modern encoder-decoder.
- [embeddings](../deeplearning/embeddings.md) — the representation layer under it all.
