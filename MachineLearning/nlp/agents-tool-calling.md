...
- **Constrained decoding** makes this reliable: grammar/JSON schema enforcement at generation time so the model *can't* emit malformed calls (inference-time, see [model-optimization-for-inference](../deeplearning/model-optimization-for-inference.md)).
- Tool results are **text** appended to the context — the model sees "the search returned: ..." and continues. Everything is still just tokens (this is why [RAG](RAG.md) and tools are the same mechanism: context assembly).

## Why it's hard

- **Long horizons**: errors compound; the model forgets its own reasoning (context is the memory — see [prompting-in-context-learning](prompting-in-context-learning.md)).
...
- **Memory**: short-term (context) vs long-term (vector store of past observations → the RAG connection again, [RAG](RAG.md)).

## Related

- [prompting-in-context-learning](prompting-in-context-learning.md) — the prompt machinery the loop runs on.
- [RAG](RAG.md) — retrieval is the simplest "tool".
- [GPT-evaluation](GPT-evaluation.md) — how to score an agent: task success rate, steps taken, cost per task.
