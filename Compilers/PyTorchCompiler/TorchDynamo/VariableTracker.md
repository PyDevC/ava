...
2. **Graph break / dynamic**: unknown Python object → can't model → break (eager fallback); unknown-but-tensor data → **guarded** (record the constraint, see [[Guards]]).

## Why it matters

- **Performance is decided by tracker fidelity**: the more Python objects Dynamo can *symbolically model* (attributes, indexing, dict/method access on known types), the fewer breaks. Custom Python classes whose attributes Dynamo can't follow are the #1 source of surprising breaks (see [[../../Programming/python/core/conditioning-on-a-python-subclass]] for the `__instancecheck__` story).
- `TORCH_LOGS=dynamo` shows tracker decisions per instruction — the practical way to see "why did this break".

## Related

- [[Trace-Lifecycle]] — where trackers are created and consumed.
- [[Guards]] — the constraints emitted when a tracker *can't* fully specialize.
- [[GraphBreaks]] — what happens when no tracker covers an op.
- [[../../Programming/python/core/conditioning-on-a-python-subclass]] — an example of tracker limits.
