...
- **`torch.export`** produces the **ExportedProgram** — the FX graph with guards + constraints (the "exported" IR going to StableHLO/IREE, see [[../../DLCompilers/XLA-StableHLO]]).
...
- **Functional** (mostly): AOTAutograd functionalizes mutations (see [[AOTAutograd]]).
- **High-level**: no loops/allocations — just ops on whole tensors.
- This is "graph IR" in the taxonomy of the [[../IR]] notes: connectivity + types, not CFG/blocks.
...
- Inspecting: `graph.print_tabular()` gives the node table — the first step in debugging any compile output (see [[../../Programming/python/core/bytecode]] for the bytecode view).

## Related

- [[../TorchDynamo/Trace-Lifecycle]] — how the FX graph is produced.
- [[AOTAutograd]] — the FX-level transformation for backward.
- [[../IR/PLAN]] — graph-IR vs CFG taxonomy.
- [[../../DLCompilers/XLA-StableHLO]] — where exported FX graphs go.
