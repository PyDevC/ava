...
- **TSAN** (thread): data races — `-fsanitize=thread`. Only with real threads (see [[python-c-api]] for the GIL-released world).
- **LSAN** (leak): comes with ASAN (`detect_leaks=1`) — finds the leaks from [[cpp-memory-model]] mistakes.
...
- Python/C++ interop: `gdb python` with the `python-gdb.py` extension → `py-bt` shows the *Python* frames under a C++ crash (invaluable for PyTorch extension crashes, see [[python-c-api]]).
- LLVM/MLIR have `LLVM_ENABLE_ABI_BREAKING_CHECKS`/assert builds — asserts are your cheapest debugger (see [[Compilers/MLIR/CMake_Guide]]).

## perf (the profiler)

- `perf record ./binary` → `perf report` — function-level CPU profile. The `perf top` live view.
- The golden rule: **profile before optimizing** (see [[python-performance]] for the same rule in Python). C++ is not "fast by default" — the hot 1% function decides everything.
- `perf stat` for counters (cache misses, branch mispredicts); `gprof` for the quick-and-dirty call graph.

## The workflow that works

1. **Reproduce** smallest-possible input (minimize — see [[OpenSource/debugging-CI-failures]] for the CI version).
...
- [[cpp-memory-model]] / [[cpp-smart-pointers]] — the bug classes sanitizers detect.
- [[OpenSource/debugging-CI-failures]] — CI logs are the same skills at scale.
- [[python-c-api]] — `py-bt` for extension crashes.
- [[Compilers/MLIR/CMake_Guide]] — build flags for assert/sanitizer builds.
