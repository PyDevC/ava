...
- **Virtualenv**: an isolated `bin/` + `site-packages/` + a `python` that points at a specific interpreter (see [frames-namespaces](core/frames-namespaces.md) for what `sys.path` actually does). `uv venv`/`python -m venv` create them; `conda` is a heavier environment manager (also handles non-Python deps).
...
- A **wheel** (`.whl`) is a *pre-built* bundle (compiled extensions included, see [python-c-api](core/python-c-api.md)) — pip just unzips it. A **sdist** (`.tar.gz`) requires building on your machine.
...
- **GPU vs CPU torch**: the CUDA wheel is a *different package* (`+cu121`); installing the CPU wheel on a GPU box silently gives slow CPU torch. This bites constantly (see [Performance](../../PyTorch/Performance.md)).
