...
CPython's memory model (refcounting, see [[python-memory]]) is not thread-safe. The GIL serializes bytecode execution so refcounts/allocations stay consistent without locks everywhere. Consequences:
...
- **3.13 free-threading**: a build option removing the GIL (`--disable-gil`/`free-threaded`). Same language, but mutable-shared-state code needs real synchronization now — the "GIL gives you safety" era is ending gradually (see the plan's note in [[core/PLAN]]).

## The DataLoader connection

`num_workers>0` in PyTorch is *processes*, precisely because data transforms are Python and must run off the main thread (see [[PyTorch/Performance]]). `pin_memory` and the transfer threads coordinate with the main loop without hitting the GIL on the hot path.

## Related

- [[python-memory]] — the refcounting the GIL protects.
- [[python-performance]] — when Python is the actual bottleneck.
- [[PyTorch/Performance]] — the practical DataLoader pattern.
- [[python-c-api]] — `Py_BEGIN_ALLOW_THREADS`, the release mechanism.
