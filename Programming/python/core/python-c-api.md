...
Writing CPython extensions is how you make Python *fast* or bridge to C++ systems (PyTorch's `torch._C` is one giant extension). Two routes: the raw **Python/C API** (`PyObject`*) and **pybind11** (modern C++ bindings). Both integrate with the execution model in [[bytecode]] and the memory model in [[python-memory]].

## The C API — `PyObject`

Everything in CPython is a `PyObject*` whose `ob_type` describes it. Extensions:

- `Py_INCREF`/`Py_DECREF` — manual refcount management (leaks → crash, see [[python-memory]]).
...
- The GIL: `Py_BEGIN_ALLOW_THREADS` / `Py_END_ALLOW_THREADS` around long-running C work — the thing that lets an extension actually parallelize (see [[gil-threading]]).
...
- The deep reason it works: it *is* the C API, generated and type-safe. For anything nontrivial (a real library, like a PyTorch custom op's CPU/CUDA path), pybind11 is the sane choice (see [[PyTorch/Custom-Ops]]).
...
- The debugging connection: a segfault in an extension = a C bug; run under **ASAN/gdb** (see [[../cpp/debugging]] plans) — Python frames in gdb via `py-bt` (Python gdb plugin).

## Related

- [[python-memory]] — refcounting you now manage by hand.
- [[gil-threading]] — `Py_BEGIN_ALLOW_THREADS` in practice.
- [[bytecode]] — what an extension module's call frame looks like.
- [[PyTorch/Custom-Ops]] — the framework-level version of this.
