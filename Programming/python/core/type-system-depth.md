...
The static side of Python typing beyond the basics in [typechecking](typechecking.md): **structural typing** (`Protocol`), **generics** (`TypeVar`), and **literal/widening types** — the tools that make type checkers actually useful on real codebases (and on the frameworks like PyTorch).
...
- The static mirror of the runtime protocol system in [data-model](data-model.md): descriptors/dunders are runtime protocols; `Protocol` makes them *checkable*.
...
- Frameworks (and Dynamo's `VariableTracker`, see [VariableTracker](../../../Compilers/PyTorchCompiler/TorchDynamo/VariableTracker.md)) distinguish static types (what mypy sees) from runtime types (`__class__`, `type(x)`) — a mismatch is a real bug source (see [conditioning-on-a-python-subclass](conditioning-on-a-python-subclass.md) for the runtime side).
...
- Type checking is a *separate tool* (mypy/pyright) from linting (ruff) — run both in CI (see [packaging](../packaging.md)/[debugging-CI-failures](../../../OpenSource/debugging-CI-failures.md)).

## Related

- [typechecking](typechecking.md) — the basics this extends.
- [data-model](data-model.md) — the runtime protocols these mirror.
- [VariableTracker](../../../Compilers/PyTorchCompiler/TorchDynamo/VariableTracker.md) — the static/runtime split in action.
