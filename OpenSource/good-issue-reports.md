...
3. **Expected vs actual**: exactly what you expected, exactly what happened (paste the full traceback — the *first* error line is gold, see [[debugging-CI-failures]]).
...
- Pasting a 500-line traceback from a giant framework crash — *minimize* to the first error (see the minimize step in [[debugging-CI-failures]]).
- Reporting a crash without version info, or a *known* issue ("the bug is in <unrelated component>").
- Forgetting you can self-triage: if your repro is solid, it's often faster to *fix it yourself* as a PR (see [[PR-Lifecycle]]).
...
- [[debugging-CI-failures]] — reading logs feeds the report.
- [[PR-Lifecycle]] — turning the report into a PR.
- [[code-review-skills]] — maintainers judge your report the way they judge code.
