...
- **The classic bug**: a non-contiguous tensor silently makes every kernel copy. `TORCH_LOGS=layout` / `tensor.stride()` / `tensor.is_contiguous()` are the diagnosis. The optimizer-fusion angle: contiguous + channels-last affects *which* kernels can fuse (see [Fusion-Scheduler](../Compilers/PyTorchCompiler/TorchInductor/Fusion-Scheduler.md)).
...
- [Dispatch-Key](Dispatch-Key.md) — dispatch reads the TensorImpl metadata.
- [Custom-Ops](Custom-Ops.md) — a custom op's CPU/CUDA impls operate on `TensorImpl`/`Storage`.
- [FX-Graph-IR](../Compilers/PyTorchCompiler/FX-Graph-IR.md) — the graph world where aliasing is functionalized away.
